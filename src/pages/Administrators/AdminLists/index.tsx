...
const AdministratorsList = (): JSX.Element => {
  ...
  const deactivateAdministrator = async (id: number) => {
    if (
      ...
      confirm(
        "This action will deactivate the selected admin account, click ok to continue"
      )
    ) {
  ...
  const activateAdministrator = async (id: number) => {
    if (
      // eslint-disable-next-line no-restricted-globals
      confirm(
        "This action will activate the selected admin account, click ok to continue"
      )
    ) {
      ...
      <div className="bg-white">
        {!viewSingle ? (
          <>
                          ...
                          <div className="col-md-6">
                            <div className="my-2">
                              <UserCard
                                ...
                                role={
                                  datum?.role
                                    ? capiitalizeFirstLetter(datum?.role?.name)
                                    : ""
                                }
                                time={"22/03/2022"}
                                avatar={
                                  datum?.avatar ? datum?.avatar : userIcon
                                }
                                active={datum?.status}
                                id={datum._id}
            ...
            />
            <div>
              <InviteAdmin addAlert={addAlert} roles={rolesArr} />
            </div>
          </>
        ) : (
          <>
