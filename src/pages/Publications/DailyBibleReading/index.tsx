...
import {
  GET_ALL_DAILY_BIBLE_READING,
  GET_ALL_BIBLE_VERSIONS,
} from "GraphQl/Queries";
import CreateButton from "utilComponents/CreateButton";
import {
  DELETE_DAILY_BIBLE_READING,
  PUBLISH_BIBLE_STUDY,
  PUBLISH_BIBLE_STUDY_READING,
  UNPUBLISH_BIBLE_STUDY,
  UNPUBLISH_BIBLE_STUDY_READING,
...
const DailyBibleReading = () => {
  ...
  const [publishStudy] = useMutation(PUBLISH_BIBLE_STUDY_READING);
  const [unpublishStudy] = useMutation(UNPUBLISH_BIBLE_STUDY_READING);
  ...
  const publishBibleSudy = async (id: number) => {
    // console.log(id);
    if (
      // eslint-disable-next-line no-restricted-globals
      confirm("This action will publish the bible study, click ok to continue")
    ) {
      await publishStudy({ variables: { biibleReadingContentId: id } });
      setState({
        alertMessage: processAlertSuccess("Bible Study Published"),
        isLoading: false,
      });
      fetchData();
    }
  };
  const unpublishBibleSudy = async (id: number) => {
    // console.log(id);
    if (
      // eslint-disable-next-line no-restricted-globals
      confirm(
        "This action will unpublish the bible study, click ok to continue"
      )
    ) {
      await unpublishStudy({ variables: { biibleReadingContentId: id } });
      setState({
        alertMessage: processAlertSuccess("Bible Study Unpublished"),
        isLoading: false,
      });
      fetchData();
    }
  ...
  const getBibleVersions = async () => {
    ...
    const apiData: any = await getAllBibleVersions?.fetchMore({
      variables: {},
    });
    if (apiData.data) {
      setState({
  ...
  return (
    <>
                    ...
                    <>
                      <div className="col-md-12 border-top border-bottom py-3">
                        <div className="d-flex row align-items-center">
                          ...
                          <div className="col-md-8 pointer border-left">
      ...
      {showCreateForm && (
        <div className="px-4">
          <CreateDailyBibleReading close={defaultView} />
        </div>
      )}
