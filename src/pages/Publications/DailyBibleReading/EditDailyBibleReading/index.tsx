...
import { EDIT_DAILY_BIBLE_READING } from 'GraphQl/Mutations';
...
const EditDailyBibleReading = (props: any):JSX.Element => {
    ...
    const [createNewDailyBibleReading, { data, loading, error }] = useMutation(EDIT_DAILY_BIBLE_READING); 
    const {fetchMore} = useQuery(GET_SINGLE_DAILY_BIBLE_READING);
    ...
    const fetchData = async() => {
        try {
            ...
            const response = await fetchMore({variables: { biibleReadingContentId: props?.bibleReadingId}});
            const { data, error }:any = response;
            if(data){
                const responseData= data?.getDailyBibleContent;
               
                setState({
                    
                    formData: {
                        bibleText: responseData?.bibleText,
                        verseOfTheDayText: responseData?.verseOfTheDayText,
            ...
            {!preview && (
                 <div className="row justify-content-between align-items-start pt-3">
                    <div className="col-md-6">
                        <PageTitle text='Update Daily Bible Reading' />
                    </div>
...
export default EditDailyBibleReading;
